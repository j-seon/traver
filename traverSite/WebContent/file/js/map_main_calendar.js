   function limitDate() { // 시작일자 선택 시 종료일자 컨트롤 활성화
      ...
      var strDate1 = sdate.value;                                 //시작일 컨트롤의 value
      var arr1 = strDate1.split('-');                              //시작일 컨트롤의 value값을  배열로
      var dat1 = new Date(arr1[0], arr1[1] - 1, arr1[2]);               //시작일 컨트롤의 value값을 Date자료형으로
      var date1 = strDate1.substring(8);                           //시작일 컨트롤의 date(일)값
      if( date1 < 10 ) { date1 = "0" + date1; }                     //date(일) 값이 10보다 작으면 앞에 0붙이기 
      edate.setAttribute("min", strDate1.substring(0,8) + date1 );      //min값 설정
      ...
      var fullEDate = dat2.toLocaleString().substring(0,12).replaceAll(". ","-").replace(".","");   //종료일 스트링형으로 변환
